...
The repository initially contains code for pulling together data from three separately published sources to create a time series of annual births on current geographies. Outputs from this process are published on the [London Datastore](https://data.london.gov.uk/dataset/annual-birth-series). The sources drawn from are:
