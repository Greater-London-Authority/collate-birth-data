...
This repository is intended to host a scripts and functions to download and process various birth datasets published by the Office for National Statistics.   

The repository initially contains code for pulling together data from four separately published sources to create a time series of monthly and annual births on current geographies. Outputs from this process are published on the [London Datastore](https://data.london.gov.uk/dataset/annual-birth-series). The sources drawn from are:
...
4. Live births by month, sex and area of usual residence of mother, England and Wales - a series of ad hoc releases of monthly births at local authority level from September 2002 onward. The URL for each release can be found in this repository's 'lookups/monthly_births_data_urls.csv' file. Recent files cover one year of data running from September to the following August.
